...
public class IntegrationMethods
{
    ...
    bool airdrag)
    {
        ...
        switch(methodsindex)
        {
            default: MidPointMethod(h, currentPosition, currentVelocity, out newPosition, out newVelocity, ref acceleratingfactor, mass, airdrag); break;
            case 0: MidPointMethod(h, currentPosition, currentVelocity, out newPosition, out newVelocity, ref acceleratingfactor, mass, airdrag); break;
            case 1: BackwardEuler(h, currentPosition, currentVelocity, out newPosition, out newVelocity, ref acceleratingfactor, mass, airdrag); break;
            case 2: ForthOrderRungeKuttaMethod(h, currentPosition, currentVelocity, out newPosition, out newVelocity, ref acceleratingfactor, mass, airdrag);break;
        ...
        bool airdrag)
    {
        ...
        if(airdrag)
            acceleratingFactor = acceleratingfactor + CalculateDrag(HalfnewVelocity, mass);   //accounting the velocity

        newVelocity = currentVelocity + h * acceleratingFactor;
        newPosition = currentPosition + h * HalfnewVelocity;

        MonoBehaviour.print(airdrag);
    }
        ...
        bool airdrag)
    {
        ...
        if(airdrag)
            acceleratingFactor = acceleratingfactor + CalculateDrag(velocity[1], mass);   //accounting the velocity
        velocity[2] = velocity[0] + h / 2.0f * acceleratingFactor;

        position[3] = position[0] + h / 2.0f * velocity[2];
        if (airdrag)
            acceleratingFactor = acceleratingfactor+ CalculateDrag(velocity[2], mass);   //accounting the velocity
        velocity[3] = velocity[0] + h / 2.0f * acceleratingFactor;

        position[4] = position[0] + h * velocity[3];
        if (airdrag)
            acceleratingFactor = acceleratingfactor+ CalculateDrag(velocity[3], mass);   //accounting the velocity
    ...
    public static Vector3 CalculateDrag(Vector3 velocityVec, float mass)
    {
        ...
        //acceleration
        float airdrag = 0.5f * vsqr * rho * c_d * A / m*100f;   //effect is hard to see here because mass is too big, *300f to magnify this effect
