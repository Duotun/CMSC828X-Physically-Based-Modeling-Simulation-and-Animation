...
public class Mouseinput : MonoBehaviour
{
    ...
    Sound Sound;
    float forceamp;
    float forceoffset;   //directly controlled in the scripts
    ...
    void HandleInput()
    {
        ...
        if (Physics.Raycast(inputRay, out hit))
        {
            ...
            if(hit.collider.gameObject.name=="Cube Metal")
            {
                Beta = 0.000000000023f;
                Alpha = 0.000020f;
                G = DelimitedReader.Read<float>("Gmatrix.csv", false, ",", false);
                D = DelimitedReader.Read<float>("Dmatrix.csv", false, ",", false);
            }
    ...
    public void forcevaluechanged(string values)
    {
        forceamp = float.Parse(values);
    }
}
