...
public class Control : MonoBehaviour
{
    ...
    void RotatingControl()
    {
        ...
        if(Input.GetMouseButtonDown(0))
        {
            ...
            StartCoroutine(IntegrationMethods_Prepare(newBullet, MassofBall, tmp, KgofPowderper * KgofPower * DeltatimeforAcceleration/ MassofBall * output.transform.up, newPosition, newVelocity, acceleratingfactor));
        }
    }

    //Parameters Preparation and then the waiting for a while the ball will be delete
    IEnumerator IntegrationMethods_Prepare(GameObject Bullet, float mass, Vector3 currentPosition, Vector3 currentVelocity, Vector3 newPosition, Vector3 newVelocity, Vector3 acceleratingFactor)
    {
        ...
        while(true)  //still in the flying
        {
            ...
            */
            ...
            IntegrationMethods.CurrentIntegrationMethod(stepsize, currentPosition, currentVelocity, out newPosition, out newVelocity,ref acceleratingFactor,mass);
